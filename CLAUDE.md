...
| `src/lib/server/**`, `src/hooks.server.ts`, `src/cli.ts`, route server files                                                     | `.claude/rules/telemetry.md` — `getLogger()` replaces `console.*`, `AppError` for throws, `@traced` / `withTrace` / `withQueryName` for spans, OTel API for metrics |
