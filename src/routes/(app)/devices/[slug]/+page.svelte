...
<div class="container mx-auto max-w-5xl px-4 py-8">
	...
	<div class="mb-6 flex items-center gap-2 text-sm text-[var(--fg-muted)]">
		...
		<span>/</span>
		<span class="text-[var(--fg)]">{data.device?.name ?? data.device?.slug}</span>
	</div>
	...
	{:else if data.device}
		...
		<div class="mb-6 flex flex-wrap items-start justify-between gap-4">
			<div>
				<div class="flex items-center gap-3">
					<h1 class="text-2xl font-bold text-[var(--fg)]">{device.name}</h1>
					<Badge variant={device.enabled ? "default" : "secondary"}>
						{device.enabled ? "enabled" : "disabled"}
					</Badge>
				</div>
				<p class="mt-1 font-mono text-sm text-[var(--fg-muted)]">{device.slug}</p>
		...
		<section class="mb-8">
			<div class="mb-3 flex items-center justify-between">
				<h2 class="text-lg font-semibold text-[var(--fg)]">Filter criteria</h2>
				{#if save_status === "saving"}
					<span class="text-xs text-[var(--fg-muted)]">Saving…</span>
		...
		<section class="mb-8">
			<h2 class="mb-3 text-lg font-semibold text-[var(--fg)]">
				...
				{#if data.images_total > 0}
					<span class="ml-1 text-sm font-normal text-[var(--fg-muted)]">
			...
			{#if data.images.length === 0}
				<p class="text-sm text-[var(--fg-muted)]">No images for this device yet.</p>
			{:else}
