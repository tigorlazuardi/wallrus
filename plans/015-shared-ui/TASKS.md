...
- [ ] `$lib/client/devices/use-devices.svelte.ts` — list hook, accepts optional `initial`, exposes `{ state, refetch }`
- [ ] `$lib/client/devices/use-device.svelte.ts` — single-device hook
- [ ] `$lib/client/devices/use-device-mutation.svelte.ts` — `{ create, update, delete, toggle }` action funcs returning parsed response
...
- [ ] Hook unit tests (`use-devices.test.ts`, `use-device-mutation.test.ts`)
